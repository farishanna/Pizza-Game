...
vector<CycleOutput> simulate(int cycles, float x=10, float y=10, float alpha = 1.1, float beta = 0.4, float delta = 0.1, float gamma = 0.4){
  vector<CycleOutput> output;
  // simulate 10 cycles
  for(int i=0; i < cycles; i++){
    // change in a timestep
    float nx = alpha * x - beta * x * y;
    float ny = delta * x * y - gamma * y;
    x = nx;
    y = ny;
    if(x < 0) x = 0;
    if(y < 0) y = 0;
...
void testCase1(){
  auto result = simulate(1, 10, 0, 1.2);
